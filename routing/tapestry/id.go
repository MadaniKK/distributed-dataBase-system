...
func SharedPrefixLength(a ID, b ID) (i int) {
	// they were actually of the same length
...
func (id ID) IsNewRoute(newId ID, currentId ID) bool {
	// TODO(students): [Tapestry] Implement me!
	if newId.String() == currentId.String() {
		return false
	}
	curSharedPrefix := SharedPrefixLength(id, currentId)
	newSharedPrefix := SharedPrefixLength(id, newId)

	// fmt.Println(curSharedPrefix, newSharedPrefix)
	// fmt.Println("localID:", id.String())
	// fmt.Println("newId:", newId.String())
	// fmt.Println("currentId:", currentId.String())

	if curSharedPrefix > newSharedPrefix {
		return false
	} else if curSharedPrefix == newSharedPrefix {
		for i := curSharedPrefix; i < DIGITS; i++ {
			diffCur := (currentId[i] - id[i] + BASE) % BASE
			diffNew := (newId[i] - id[i] + BASE) % BASE
			if diffCur != diffNew {
				return diffNew < diffCur
			}
		}
		// if till 40th digit newId is the same as currId, return false
		return false
...
func (id ID) Closer(a ID, b ID) bool {
	...
	aBig := a.Big()
	bBig := b.Big()
	aDiff := new(big.Int).Sub(aBig, idBig)
	bDiff := new(big.Int).Sub(bBig, idBig)
	return aDiff.CmpAbs(bDiff) == -1
