...
func (s *State[T]) safelyUpdateKey(newKV *conflict.KV[T]) (updated bool, mostUpToDateKV *conflict.KV[T], err error) {
	...
	tx := localStore.BeginTx(false)
	if localKV, ok := tx.Get(newKV.Key); ok {
		// if newKV clock happens before localKey then not update and return local value
		if newKV.Clock.HappensBefore(localKV.Clock) {
			err := tx.Commit()
			return false, localKV, err
		} else if localKV.Clock.HappensBefore(newKV.Clock) {
			tx.Put(newKV.Key, newKV)
			err := tx.Commit()
			return true, newKV, err

		} else {
			conflicResolver := s.conflictResolver
			winningKV, err := conflicResolver.ResolveConcurrentEvents(localKV, newKV)
			if err != nil {
				log.Fatalf("Error resolving the conflict: {%v}", err)
			}
			if winningKV.Value == newKV.Value {
				updated = true
			} else {
				updated = false
			}
			tx.Put(winningKV.Key, winningKV) // they have the same key actually
			err = tx.Commit()
			return updated, winningKV, err

		}
		// if local KV happens before the the newKV then we update

		// if they are concurrent, resolve conflict and get the largest value
		// if !newKV.Clock.HappensBefore(localKV.Clock) && !localKV.Clock.HappensBefore(newKV.Clock) {

		// }
		// return false, localKV, errors.New("not implemented")

	} else {
		// if newKey does not exist, just create it in store
		tx.Put(newKV.Key, newKV)
		err := tx.Commit()
		return true, newKV, err
...
func (s *State[T]) getUpToDateKV(key string, minimumClock T) (kv *conflict.KV[T], found bool) {
	localStore := s.localStore
	localStore.Get(key)
	tx := localStore.BeginTx(true)
	if localKV, ok := tx.Get(key); ok {
		// localclocks ealier than minimum clocks, we do not return
		if localKV.Clock.HappensBefore(minimumClock) {
			...
			// if local KV not happens before minimum,
			//means they equal / concurrent /minimum happens before localClock
			return localKV, true

		}

	} else {
		//we have to commit the tx even when is readOnly
		err := tx.Commit()
		if err != nil {
			log.Fatal(err)
		}
...
func (s *State[T]) HandlePeerWrite(ctx context.Context, r *pb.ResolvableKV) (*pb.HandlePeerWriteReply, error) {
	...
	accepted, mostUpToDateKV, err := s.safelyUpdateKey(newKV)
	...
	writeReply := &pb.HandlePeerWriteReply{
		Accepted:     accepted,
...
func (s *State[T]) replicateToNode(ctx context.Context, kv *conflict.KV[T], replicaNodeID uint64) error {
	...
	// resolvableKV := &pb.ResolvableKV{
	// 	Key:   kv.Key,
	// 	Value: kv.Value,
	// 	Clock: kv.Proto().Clock,
	// }

	resolvableKV := kv.Proto()
	if resolvableKV == nil {
		s.log.Printf("sth wrong with replicateToNode %s", resolvableKV)
	}

	f := func() error {
		// should the life cycle be called three times?
		s.onMessageSend()
		wr, err := replicaRPCClient.HandlePeerWrite(ctx, resolvableKV)
		if err != nil {
			s.log.Printf("sth wrong with handlePeerWrite %s", err)
			...
			if wr != nil {
				if wr.Accepted {
					...
					// err = errors.New("the replica rejected our write")
					s.log.Printf("the replica rejected our write ")
...
func (s *State[T]) ReplicateKey(ctx context.Context, kv *pb.PutRequest) (*pb.PutReply, error) {
	...
	if newKV == nil {
		s.log.Printf("sth wrong with newKV in replicate Key %s", newKV)
	}
	// replicating to itself : i think it is written in dispatchToPeers
	// accepted, mostUpToDateKV, err := s.safelyUpdateKey(newKV)

	f := func(ctx context.Context, replicaNodeID uint64) error {
		err := s.replicateToNode(ctx, newKV, replicaNodeID)
		if err != nil {
			s.log.Printf("sth wrong with replicateToNode %s", err)

		}
		return err

	}
	err := s.dispatchToPeers(ctx, s.W, f)

	if err == nil {
		putReply := &pb.PutReply{
			Clock: newKV.Proto().Clock,
		}
		return putReply, nil
...
func (s *State[T]) HandlePeerRead(ctx context.Context, request *pb.Key) (*pb.HandlePeerReadReply, error) {
	...
	returnedKV, exist := s.getUpToDateKV(requestKey, requestClock)

	if !exist { // key is not found
		return nil, nil
		//errors.New("This key does not exist in this node")
	} else {
		if returnedKV == nil {
			// key exisist but not uptodate
			...
			// has an uptodate enough localKV
			readReply := &pb.HandlePeerReadReply{
				Found:        true,
				ResolvableKv: returnedKV.Proto(),
			}
			return readReply, nil
...
func (s *State[T]) readFromNode(ctx context.Context, key string, replicaNodeID uint64, clientClock T) (*conflict.KV[T], error) {
	...
	// var returnedKV *conflict.KV[T]
	returnedKV := new(conflict.KV[T])
	//&{key: "", value: "", clock: {}}

	f := func() error {
		s.onMessageSend()
		readReply, err := replicaRPCClient.HandlePeerRead(ctx, request)
		if readReply == nil {
			// key doesnt exist
			return nil
		}
		// key not uptodate enough
		if readReply.ResolvableKv == nil {
			return nil

		}
		// returnedKV := conflict.KVFromProto[conflict.Clock](readReply.ResolvableKv)
		returnedKV = conflict.KVFromProto[T](readReply.ResolvableKv)
		//err is always nill
		return err
	...
	err := s.withRetries(f, 3)
	if err != nil {
		s.log.Printf("sth wrong with the readRPC,%v", err)
	}
	if returnedKV.Key == "" {
		//if key does not exist or not uptodate enough return nil,err
		return nil, err
	}
	return returnedKV, err
...
func (s *State[T]) PerformReadRepair(ctx context.Context, latestKV *conflict.KV[T], kvPairs map[uint64]*conflict.KV[T]) {
	...
	for nodeID, nodeKV := range kvPairs {
		// no need to update: when kv is not nil and latestKV's clock happens before node's clock
		if nodeKV != nil && latestKV.Clock.HappensBefore(nodeKV.Clock) {
			continue
		}
		// other cases: nodeKV is nil / nodeKV is concurrent or before latestKV
		wg.Add(1)
		go func(nodeID uint64, latestKV *conflict.KV[T]) {
			defer wg.Done()
			// //should we check their uptodateness here?  or its done in safelyupdates?
			// err := s.replicateToNode(ctx, latestKV, nodeID)
			// if err != nil {
			// 	s.log.Printf("node %d reject our write", nodeID)
			// }

			// Send writes to node to update to latestKV
			s.log.Printf("Read repair: write to node being called for node %d", nodeID)
			conn := s.node.PeerConns[uint64(nodeID)]
			replicaRPCClient := pb.NewBasicLeaderlessReplicatorClient(conn)
			s.onMessageSend()
			wr, err := replicaRPCClient.HandlePeerWrite(ctx, latestKV.Proto())
			if err != nil {
				s.log.Printf("Read repair: sth wrong with handlePeerWrite %s", err)
			} else {
				if wr != nil {
					if !wr.Accepted {
						s.log.Printf("Read repair: the replica rejected our write")
					}
				}
			}
		}(nodeID, latestKV)
...
func (s *State[T]) GetReplicatedKey(ctx context.Context, r *pb.GetRequest) (*pb.GetReply, error) {
	...
	repliedKVs := make([]*conflict.KV[T], 0, s.R)
	kvPairs := make(map[uint64]*conflict.KV[T])
	var mu sync.Mutex
	f := func(ctx context.Context, replicaNodeID uint64) error {
		replicaKV, err := s.readFromNode(ctx, r.Key, replicaNodeID, clientClock)
		if err != nil {
			return err
		}

		// Lock to safely update the variables
		mu.Lock()
		defer mu.Unlock()

		// Add kv from node's reply to map
		// (the kv can be nil, when key does not exist or not up to date)
		kvPairs[replicaNodeID] = replicaKV

		// // Add the replied KV to the list
		// // Update the most up-to-date KV
		// //replicakv is nil when the key not exist and not uptodate enough
		if replicaKV != nil {
			repliedKVs = append(repliedKVs, replicaKV)
		}
		// if len(repliedKVs) != 0 {
		// 	winningKV, err = s.conflictResolver.ResolveConcurrentEvents(repliedKVs...)
		// }
		return err
	}
	err := s.dispatchToPeers(ctx, s.R, f)
	if err != nil {
		s.log.Printf("sth wrong with dispatchToPeers, %v", err)
	}

	mu.Lock()
	// key doesnt exist // all the nodes have no uptodate value
	if len(repliedKVs) == 0 {
		return nil, errors.New("the key does not exist, or no up to date value found in partition")
	}
	// find the winning kv (as up to date as all other kvs, and have largest value)
	winningKV := &conflict.KV[T]{}
	winningKV.Clock = s.conflictResolver.ZeroClock()
	for _, kv := range repliedKVs {
		if kv.Clock.HappensBefore((winningKV.Clock)) {
			continue
		}
		if winningKV.Clock.HappensBefore(kv.Clock) {
			winningKV = kv
		} else {
			// we will ignore err here because we won't pass in empty conflicts
			winningKV, _ = s.conflictResolver.ResolveConcurrentEvents(winningKV, kv)
		}
	}
	mu.Unlock()

	// perform readrepair
	s.PerformReadRepair(ctx, winningKV, kvPairs)

	// assemble reply to client
	getReply := &pb.GetReply{
		Value: winningKV.Value,
		Clock: winningKV.Clock.Proto(),
	}
