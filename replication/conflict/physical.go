...
/*
In order to compare the PhysicalClock instance with the other parameter, the other parameter
needs to be converted to a PhysicalClock type. This is done using a type assertion,
which is a way to extract a value of a specific type from an interface value.
*/
...
func (c *PhysicalClockConflictResolver) ResolveConcurrentEvents(conflicts ...*KV[PhysicalClock]) (*KV[PhysicalClock], error) {
	...
	var maxConflict *KV[PhysicalClock]
	// compare by
	for _, conflict := range conflicts {
		if maxConflict == nil || conflict.Value > maxConflict.Value {
